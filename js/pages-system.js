...
Pages.inventoryOverview = function () {
    ...
    <div class="table-wrap list-scroll">
        <table class="table">
            <thead><tr><th>品号</th><th>品名</th><th>规格</th><th>仓库</th><th>币别</th><th class="num">采购成本</th><th class="num">成本(本位币)</th><th>库存单位</th><th>目前库存</th><th class="num">安全库存</th><th>状态</th><th class="num">库存价值(本位币)</th><th class="num">汇率</th><th>最后异动</th></tr></thead>
...
Pages.clearBusinessData = function () {
    confirmModal(
        "确定要清空全部业务数据吗？将删除：销货单、出货单、采购单、库存调整、销货退回/折让、采购退回/折让、商品主档及全部库存（安全库存一并清零）。<br><br>系统账号、客户、供应商、仓库、币别、单位、分类等基础资料会保留。此操作不可撤销！",
        () => {
            DB.clearBusiness();
