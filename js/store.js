...
const DB = {
    ...
    /* 损益报表权限迁移（2026-08-28）：为已拥有报表权限（report.*）的角色补入损益报表权限（幂等）。
       不授予与报表无关的角色（如仅 dashboard.view 的新建角色），避免无差别扩散经营敏感数据。 */
    migrateProfitReport() {
        let dirty = false;
        (this._mem.roles || []).forEach(r => {
            const perms = r.permissions || [];
            if (perms.includes("report.profit")) return;
            if (perms.some(p => p.indexOf("report.") === 0)) {
                perms.push("report.profit");
                dirty = true;
            }
        });
        if (dirty) this.flush();
...
const PERMISSIONS = [
    ...
    { code: "inventory.view", label: "查看样品领料", group: "日常作业" },
    { code: "inventory.adjust", label: "执行样品领料", group: "日常作业" },
    ...
    { code: "finance.ar", label: "进销存应收账款", group: "进销存账款" },
    { code: "finance.ap", label: "进销存应付账款", group: "进销存账款" },
    { code: "finance.bookkeeping", label: "财务会计（外挂记账系统）", group: "进销存账款" },
    { code: "finance.quote", label: "商品估价试算（外挂跨境成本利润试算）", group: "进销存账款" },
